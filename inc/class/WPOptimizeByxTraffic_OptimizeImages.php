...
class WPOptimizeByxTraffic_OptimizeImages extends WPOptimizeByxTraffic_Base 
{
	...
	private function optimize_images_fixFileName($file_name)
	{
		...
		$file_name = preg_replace('#[\s \t]+#is','-',$file_name);
	...
	public function optimize_images_process_image($input_parameters)
	{
		...
		if(isset($paramsWatermarkOptions['text']) && $paramsWatermarkOptions['text']) {
			if(is_array($paramsWatermarkOptions['text']) && !empty($paramsWatermarkOptions['text'])) {
				$checkStatus1 = true;
			}
		}
		...
		if(isset($paramsWatermarkOptions['image']) && $paramsWatermarkOptions['image']) {
			if(is_array($paramsWatermarkOptions['image']) && !empty($paramsWatermarkOptions['image'])) {
				$checkStatus1 = true;
			}
		}
		...
		*/
		...
		if($imgOptimizedFilePathExists1) {
			...
			$rs_getimagesize = getimagesize($imgOptimizedFilePathExists1);
			...
			if(PepVN_Data::isUrl($input_parameters['original_image_src'])) {
				...
				if(file_exists($input_parameters['optimized_image_folder_path'])  && PepVN_Data::isAllowReadAndWrite(PepVN_Data::getFolderPath($input_parameters['optimized_image_folder_path']))) {
							...
							if(file_exists($imgOptimizedFilePath2)) {
								if(filesize($imgOptimizedFilePath2)>0) {
									...
									if($filemtimeTemp1) {
										...
										if((PepVN_Data::$defaultParams['requestTime'] - $filemtimeTemp1) <= (86400 * 7)) {
											return $resultData;
										} else {
					...
					if(!$resultData['image_optimized_file_path']) {
						...
						if($imgFolderCachePath && file_exists($imgFolderCachePath) && is_dir($imgFolderCachePath)) {
						} else {
						...
						if($imgFolderCachePath && file_exists($imgFolderCachePath)  && PepVN_Data::isAllowReadAndWrite(PepVN_Data::getFolderPath($imgFolderCachePath))) {
							...
							if(!file_exists($imgOriginalCacheFilePath)) {
								...
								$imgSrcContent = $this->quickGetUrlContent(
									$input_parameters['original_image_src']
									, array(
										'cache_timeout' => 86400
									)
								);
								...
								if($valueTemp1 && ($valueTemp1>0)) {
									...
									if($this->optimize_images_IsImageFilePathCanProcess($imgOriginalCacheFilePath)) {
										...
										if(isset($imgOriginalCacheFile_RsGetImageFileInfo['image_type']) && $imgOriginalCacheFile_RsGetImageFileInfo['image_type']) {
											...
											$pepVN_PHPImage = 0; unset($pepVN_PHPImage);
										}
									...
									if($options['optimize_images_watermarks_watermark_image_url']) {
										...
										if(PepVN_Data::isUrl($options['optimize_images_watermarks_watermark_image_url']) && PepVN_Data::isImg($options['optimize_images_watermarks_watermark_image_url'])) {
											...
											if(!file_exists($imgWatermarkFilePath1)) {
												...
												$valueTemp1 = $this->quickGetUrlContent(
													$options['optimize_images_watermarks_watermark_image_url']
													, array(
														'cache_timeout' => 86400
													)
												); 
	...
	public function optimize_images_get_patterns_for_parse_custom_tag_of_image($input_parameters)
	{
		$resultData = false;
		...
		if(isset($input_parameters['post_id']) && $input_parameters['post_id']) {
			...
			if($input_parameters['post_id'] > 0) {
				$postObj = get_post($input_parameters['post_id']);
			}
		} else {
			global $post;
			if($post) {
		...
		if($postObj) {
			if(isset($postObj->ID) && $postObj->ID) {
				...
				if(!$resultData) {
					...
					$rsGetTerms = $this->base_get_terms_by_post_id($postObj->ID);
	...
	*/
	...
	public function optimize_images_parse_custom_tag_of_image($input_parameters)
	{
		...
		$input_raw_tag = PepVN_Data::$cacheObject->get_cache($keyCache1);
		...
		if(null === $input_raw_tag) {
			...
			if(isset($input_parameters['raw_tag']) && $input_parameters['raw_tag']) {
				...
				$patterns = $this->optimize_images_get_patterns_for_parse_custom_tag_of_image($input_parameters);
				...
				$input_raw_tag = PepVN_Data::removeQuotes($input_raw_tag);
				$input_raw_tag = preg_replace('#^[\- \s]+#is','',$input_raw_tag);
				$input_raw_tag = preg_replace('#[\- \s]+$#is','',$input_raw_tag);
				$input_raw_tag = preg_replace('#[\-]+[\s ]+[\-]+#is',' - ',$input_raw_tag);
				
				$input_raw_tag = PepVN_Data::reduceSpace($input_raw_tag);
				$input_raw_tag = trim($input_raw_tag);
				
			}
			
			PepVN_Data::$cacheObject->set_cache($keyCache1, $input_raw_tag);
		}
	...
	{
		return $this->advancedCacheObject->optimize_images_check_get_screen_width();
	}
	
	
	public function optimize_images_check_set_size_image_tag(&$image_tag_text, $image_size, $override_status = false)
	{
		...
		$valueTemp = PepVN_Data::$cachePermanentObject->get_cache($keyCache1);
	...
	public function optimize_images_check_get_max_size_image_tag(&$image_tag_text)
	{
		$resultData = array(
			'width' => 0
			,'height' => 0
		...
		$screen_width = $this->optimize_images_check_get_screen_width();
	...
	public function optimize_images_process_text($text)
	{
		...
		if(
			$options['optimize_images_optimize_image_file_enable']
		...
		$keyCacheProcessText = array(
			...
			,$text
			,'process_text'
			,'device_screen_width_'.$device_screen_width
		);
		...
		$keyCacheProcessText = PepVN_Data::fKey(array(
			$keyCacheProcessText
			,$rsSettingWatermarksFirstOptions['paramsWatermarkOptions']
		));
		
		$rsSettingWatermarksFirstOptions = 0;
					...
					if($value1) {
						...
						if(isset($matched2[2]) && $matched2[2]) {
							...
							if($matched2) {
								...
								$rsProcessImage1 = $this->optimize_images_process_image($processImageOptions1);
								
								$processImageOptions1 = 0;
		...
		if(preg_match_all('#<img[^>]+\\\?>#i', $text, $matched1)) {
			...
			if(isset($matched1[0]) && is_array($matched1[0]) && (!empty($matched1[0]))) {
				...
				foreach($matched1 as $keyOne => $valueOne) {
					...
					$optimize_images_alttext1 = $this->optimize_images_parse_custom_tag_of_image(array(
					...
					$optimize_images_titletext1 = $this->optimize_images_parse_custom_tag_of_image(array(
					...
					if($imgSrc) {
						...
						if($checkStatus2) {
							...
							if($options['optimize_images_rename_img_filename_value']) {
								...
								$optimize_images_rename_img_filename_value = $this->optimize_images_parse_custom_tag_of_image(array(
							...
							$max_size_image_tag = $this->optimize_images_check_get_max_size_image_tag($oldImgTag);
								...
								if(file_exists($imgOptimizedFilePathExists1)) {
									if(filesize($imgOptimizedFilePathExists1)>0) {
										...
										if(isset($rs_getimagesize1[0]) && $rs_getimagesize1[0]) {
											$newImgTag = $this->optimize_images_check_set_size_image_tag($newImgTag,array(
	...
	*/
	...
	public function optimize_images_the_content_filter($text) 
	{
		...
		return $this->optimize_images_process_text($text);
	}
	...
	public function optimize_images_handle_options()
	{
		...
		$optimize_images_auto_resize_images_enable = $options['optimize_images_auto_resize_images_enable'] === 'on' ? 'checked':'';
				...
				<form name="WPOptimizeByxTraffic" action="',$action_url,'" method="post">
					  <input type="hidden" id="_wpnonce" name="_wpnonce" value="',$nonce,'" />
						...
						<div id="xtraffic_tabs_content1" class="xtraffic_tabs_contents">
							...
							<ul>
								<li>%title : ',__('post title or WooCommerce product title',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),'</li>
								<li>%category : ',__('post category',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),'</li>
								<li>%tags : ',__('post tags',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),'</li>
								<li>%product_tag : ',__('WooCommerce product tags',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),'</li>
								<li>%product_cat : ',__('WooCommerce product categories',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),'</li>
							...
							<div id="optimize_images_optimize_file_container">
								...
								<ul>
									...
									<li>
										...
										<h3>',__('Automatically resize the images',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),' :</h3>
										<ul>
											<li>
												<input type="checkbox" name="optimize_images_auto_resize_images_enable" class="" ',$optimize_images_auto_resize_images_enable,' value="on" /> &nbsp; 
												<label for="optimize_images_auto_resize_images_enable"> ',__('Enable "Auto Resize Images to fit the screen width of the device"',WPOPTIMIZEBYXTRAFFIC_PLUGIN_SLUG),'</label> 
											</li>
										</ul>
